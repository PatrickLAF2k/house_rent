    ...
    describe('POST /register', () => {
        ...
        describe('Validação do campo date_of_birth', () => {
            ...
            it('não deve registrar um usuário com data de nascimento inválida', async () => {
                ...
                expect(response.statusCode).toBe(400);
                expect(response.body).toHaveProperty('mensagem', 'Data de nascimento inválida');
            });

            it('não deve registrar um usuário com data de nascimento fora do formato ISO', async () => {
                const response = await request(app)
                    .post('/register')
                    .send({
                        name: 'Owner Test',
                ...
                expect(response.statusCode).toBe(400);
                expect(response.body).toHaveProperty('mensagem', 'A data de nascimento deve estar no formato ISO (DD-MM-AAAA)');
            });

            it('não deve registrar um usuário sem preencher o campo data de nascimento', async () => {
                const response = await request(app)
                    .post('/register')
                    .send({
                        name: 'Owner Test',
